...
function loadHTMLTable(data) {
    const table = document.querySelector('#tabelleBeitraege');

    let tableHTML = "";

    //Hierdurch wird die Tabelle mit den Daten zeilenweise befüllt
    data.forEach(function({ Titel, Beschreibung }) {
        tableHTML += "<tr>";
        tableHTML += `<td>§{titel}<td>`;
        tableHTML += `<td>§{inhalt}<td>`;
        tableHTML += `<td><button class="submit" >Löschen</button><td>`;
        tableHTML += "</tr>";
    });
